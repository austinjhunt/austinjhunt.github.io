...
export const skeleton = ({ width = null, height = null, style = {}, shape = "rounded-full", className = null }) => {
  return (
    <div
      className={`bg-base-300 animate-pulse ${shape}${className ? ` ${className}` : ""}${width ? ` ${width}` : ""}${
        height ? ` ${height}` : ""
      }`}
      style={style}
    />
...
export const sanitizeConfig = (config) => {
  ...
  return {
    ...
    degree: {
      ...
      school: config?.degree?.school || "",
      degreeFileUrl: config?.degree?.degreeFileUrl || "",
...
export const notFoundError = {
  ...
  subTitle: (
    <p>
      Please provide correct github username in <code>gitprofile.config.js</code>.
    </p>
  ),
};
